...
| [POV-012 Loopback LLM text round trip](tickets/POV-012-loopback-llm-text-round-trip.md) | Delivery | Completed — 2026-07-31; Core/API/Web, fake provider, actual local model과 locked baseline PASS | 저장→queue→authenticated loopback inference→atomic assistant/provenance append; model은 narrow candidate이고 production activation은 POV-038에 유지 |
| [POV-013 Conversation Core offline evidence gate](tickets/POV-013-conversation-core-offline-evidence-gate.md) | Evidence gate | In Progress — `fix`; POV-039/040 repaired 2026-08-01 | clean/offline/frontend와 core invariants는 PASS; full matrix rerun과 target-Chrome evidence가 남음 |
| [POV-039 Repair job status browser evidence fixture](tickets/POV-039-repair-job-status-browser-evidence-fixture.md) | Corrective test | Completed — 2026-08-01; browser `1/1`, unit `13/13` PASS | stale Playwright fixture를 current owner-scoped schema와 맞추고 production behavior를 유지 |
| [POV-040 Reconcile macOS raw filename validation](tickets/POV-040-reconcile-macos-raw-filename-validation.md) | Corrective platform validation | Completed — 2026-08-01; native Terminal locked baseline PASS | APFS EILSEQ capability를 test-only로 분류하고 raw/unknown artifact fail-closed 계약을 유지 |
...
| [POV-038 macOS dogfood runtime and installed-browser evidence](tickets/POV-038-macos-dogfood-runtime-and-installed-browser-evidence.md) | Platform activation evidence | In Progress — target available; manual evidence pending | macOS always-on/dogfood production 및 installed-browser 지원을 주장하기 전 |

POV-010은 자동화된 component·HTTP contract와 supported-Unix smoke로 구현 delivery를
완료했습니다. POV-040 target-macOS baseline도 완료했지만 production `auth init`, listener와
installed-browser login/refresh/logout·cookie/storage evidence는 POV-038에서 실행합니다.
...
| H1 — Trustworthy text capture | In Progress — POV-013 `fix`; POV-039/040 completed, full rerun required | 한 owner가 offline에서 text를 durable하게 남기고 retry·status·local inference failure를 신뢰 | [POV-007](deps/POV-007-local-login-refresh-and-session-revoke.md), [POV-008](deps/POV-008-idempotent-conversation-append-and-outbox.md), [POV-009](deps/POV-009-durable-single-slot-job-queue.md), [POV-010](tickets/POV-010-minimal-authenticated-local-text-chat.md), [POV-011](deps/POV-011-authenticated-replayable-job-status-stream.md), [POV-012](tickets/POV-012-loopback-llm-text-round-trip.md), [POV-013](tickets/POV-013-conversation-core-offline-evidence-gate.md), [POV-039](tickets/POV-039-repair-job-status-browser-evidence-fixture.md), [POV-040](tickets/POV-040-reconcile-macos-raw-filename-validation.md) |
...
- [x] [ADR-0006](decisions/0006-h1-development-and-dogfood-platform.md)에서 MacBook
  ...
  환경으로 결정
- [ ] [POV-038](tickets/POV-038-macos-dogfood-runtime-and-installed-browser-evidence.md)에서 target
  MacBook production auth와 installed-browser dogfood evidence 실행; 선행 POV-040 baseline은 완료
...
- 2026-08-01: [POV-040](tickets/POV-040-reconcile-macos-raw-filename-validation.md) native APFS의 raw filename `EILSEQ`를 test-only capability boundary로 보정하고 동일 native Terminal에서 raw tests, process suite와 locked workspace baseline을 통과.
- 2026-08-01: [POV-039](tickets/POV-039-repair-job-status-browser-evidence-fixture.md) stale Playwright status fixture에 owner-scoped linkage를 복구하고 current Chromium, Web unit과 frontend baseline을 통과; POV-013 전체 재실행은 계속 pending.
- 2026-07-31: [POV-012](tickets/POV-012-loopback-llm-text-round-trip.md) outbox scanner, authenticated loopback provider, atomic assistant/provenance, generation status/cancel UI와 fake/actual-model round-trip delivery 완료; model은 narrow candidate로 유지.
