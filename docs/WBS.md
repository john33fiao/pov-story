...
| 9 | [POV-011 Authenticated replayable job status stream](tickets/POV-011-authenticated-replayable-job-status-stream.md) | In Progress — implementation/Chromium과 actual macOS locked check 및 재실행 full test PASS; post-fix Linux 검증 대기 | POV-009, POV-010 | token refresh와 reconnect 뒤에도 owner-scoped 상태를 이어 봄; Linux locked suite와 auth/PTY/production smoke 뒤 종료 |
