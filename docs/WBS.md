...
| 9 | [POV-011 Authenticated replayable job status stream](tickets/POV-011-authenticated-replayable-job-status-stream.md) | In Progress — implementation/Chromium evidence present; actual macOS locked Rust baseline blocked by pre-existing `Termios.line_discipline` E0609 | POV-009, POV-010 | token refresh와 reconnect 뒤에도 owner-scoped 상태를 이어 봄; supported-Unix baseline 복구 뒤 종료 |
