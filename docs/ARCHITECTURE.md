...
- `pov-api`는 아직 `pov-core`에 의존하거나 store를 열지 않습니다. auth verifier, Conversation append/queue endpoint, Knowledge/Calendar domain adapter, production instance-root config/store wiring, Blob, background dispatcher/worker, SSE, 실제 provider/model과 external ingress도 구현하지 않았습니다.
...
- process-local permit은 live runtime의 child 실행 겹침을 막습니다. POV-009의 Conversation DB queue core가 crash-recoverable FIFO, fenced lease와 durable system-wide admission state를 보존하지만, 이 state를 `ProcessSupervisor` 또는 provider와 연결해 계속 실행하는 runtime loop는 POV-010~012 통합 전까지 없습니다.
...
[ADR-0004](decisions/0004-local-authentication-and-session-security-contract.md)가 auth/session 계약을 accepted했습니다. account, password/recovery verifier, durable throttle, login-attempt marker/outcome, session, active refresh family/predecessor와 auth audit source는 Conversation DB의 control-plane auth namespace에 두고 signing private key는 DB 밖 owner-only secret directory에 둡니다. Migration `0004`/`0005`의 dependency-independent schema, private auth mutation executor, credential primitive, crate-private canonical keyring codec, pure transition-artifact/initialization-metadata v1 codec와 Unix instance-directory/maintenance-lock/store-binding/detached actor, read-only initialization reconciliation, initialization-only pre-source preparation·recovery·exact rollback, actor-owned exact `Prepared → initializing` source-CAS와 pre-writer durability fence, active-key install, final `initializing → active` lifecycle CAS와 deletion-only cleanup primitive는 구현됐습니다. Planned/retire/compromise/loss transition, production auth mutation repository, issuer/verifier, HTTP와 runtime wiring은 아직 구현하지 않았습니다.
